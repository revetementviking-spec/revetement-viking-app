      ...
      if (existant) {
        if (!existant.paye) {
          ...
          if (!inchangee) ecritures.push({
            sql: `UPDATE paies_periodes SET heures_normales=?, heures_sup=0, heures_travaillees=?, banque_dispo=?, banque_appliquee=?, banque_solde=?, taux_horaire=?, das_pct=?, montant_brut=?, das_montant=?, montant_net=? WHERE id=?`,
            args: [payees, travaillees, dispoAvant, appliquee, banque, taux, dasPct, brut, dasMontant, net, existant.id],
          });
        } else {
        ...
        ecritures.push({
          ...
          args: [g.employe, g.debut, g.fin, payees, travaillees, dispoAvant, banque, taux, dasPct, brut, dasMontant, net, new Date().toISOString()],
        });
      }
    }
  }
